...
def create(
    ...
    **payload,
    ...
    session: AuthorizedSession = session_manager.session
    data: str
...
def read(
    ...
    if not registry.check_capability(cls=model, capability=registry.Capability.get):
        raise NotImplementedError(
            f"{registry.Capability.get} not implementend or supported by {model.__name__}"
        )
...
def update(
    ...
    **payload,
    ...
    if (
        model_data
        and model_data.__class__ in registry._MODEL_REGISTRY.keys()
        and registry.check_capability(
            cls=model_data.__class__, capability=registry.Capability.update
        )
        and not payload
    ):
        obj = model_data
        ...
        model
        and model in registry._MODEL_REGISTRY.keys()
        and registry.check_capability(
            cls=model_data.__class__, capability=registry.Capability.update
        )
...
def patch(
    ...
    **payload,
    ...
    """
    Patch-Updates a Google Wallet Class or Object. `U` in CRUD.

    :param model:              model to be patched
    :param resource_id:        resource id of object / class which should be patched
    :param payload:            data to pass to the Google RESTful API
    :raises LookupError:       if the resource was not found (404)
    :raises Exception:         if the response status code is not 200 or 404
    :return:                   the created model based on the data returned by the API
    ...
    if model in registry._MODEL_REGISTRY.keys() and registry.check_capability(
        cls=model, capability=registry.Capability.patch
    ):
        ...
        if response.status_code == 404:
            raise LookupError(f"{model.__name__} with id: {resource_id} not found")
        ...
        return model.model_validate_json(response.content)
    raise ValueError(
        f"{model.__name__} not known or does not support {registry.Capability.patch}"
    )
...
def list(
    ...
    *,
    ...
    if model in registry._MODEL_REGISTRY.keys() and registry.check_capability(
        ...
        params = {}
        if model.__name__.endswith("Class"):
            params["issuerId"] = resource_id
        elif model.__name__.endswith("Object"):
        ...
        if response.status_code == 200:
            ...
            if pagination.resultsPerPage > 0:
                ...
                for elem in raw_objs:
                    try:
                        obj = model.model_validate(elem)
                        objs.append(obj)
                    except Exception as e:
                        # breakpoint()
                        print(e)
        elif response.status_code == 404:
            raise LookupError(f"{model.__name__}: {resource_id} not found")
        else:
