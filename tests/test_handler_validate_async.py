"""Async tests for handlers.validate module.
...
@pytest.mark.asyncio
async def test_google_public_key_cached_empty_async(mock_settings):
    ...
    keys = await google_root_signing_public_keys_async(mock_settings.google_environment)
    ...
    keys2 = await google_root_signing_public_keys_async(
...
async def test_handler_validate_ok_async(mock_settings):
    """Test successful signature validation (async)."""
    from edutap.wallet_google.handlers.validate import verified_signed_message_async
...
@pytest.mark.asyncio
async def test_handler_validate_invalid_async(mock_settings):
    ...
    with pytest.raises(Exception):
        await verified_signed_message_async(data)


@pytest.mark.asyncio
@freeze_time("2025-10-01 12:00:00")
async def test_message_expiration_valid_async(mock_settings):
...
async def test_message_expiration_expired_async(mock_settings):
...
async def test_message_expiration_just_before_expiry_async(mock_settings):
...
async def test_message_expiration_expiry_check_ignored_async(mock_settings):
    """
...
async def test_cache_expiration_refresh_async(mock_settings):
    ...
    keys1 = await google_root_signing_public_keys_async(
        mock_settings.google_environment
    )
    ...
    keys3 = await google_root_signing_public_keys_async(
        mock_settings.google_environment
    )
...
@pytest.mark.asyncio
async def test_signature_verification_disabled_async(mock_settings):
...
@pytest.mark.asyncio
@freeze_time("2025-10-01 12:00:00")
async def test_protocol_version_mismatch_async(mock_settings):
